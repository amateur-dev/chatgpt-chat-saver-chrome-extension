// Requires html2canvas (your 1.4.1) + jsPDF >= 2.5.1 in your extension
import html2canvas from './libs/html2canvas.min';
import { jsPDF } from 'jspdf';

export async function saveElementAsPdf(el: HTMLElement, filename = 'chat.pdf') {
  const canvas = await html2canvas(el, {
    ...
    scrollY: 0,
    onclone: (doc) => {
      // Unstick sticky/fixed to avoid overlaps; expand scroll containers
      doc.querySelectorAll<HTMLElement>('*').forEach(node => {
        const cs = (doc as any).defaultView.getComputedStyle(node);
        if (cs.position === 'sticky' || cs.position === 'fixed') node.style.position = 'static';
      ...
      // Narrow width for stable line wrapping
  ...
  // A4 at 96dpi ≈ 794 x 1123 px. Adjust if you want Letter (816x1056).
  const PX_PER_PAGE = { w: 794, h: 1123 };
  const pdf = new jsPDF({ unit: 'px', format: [PX_PER_PAGE.w, PX_PER_PAGE.h], hotfixes: ['px_scaling'] });

  const pageW = PX_PER_PAGE.w;
  const scale = pageW / canvas.width;
  const pageHInCanvasPx = Math.floor(PX_PER_PAGE.h / scale);
  ...
  while (y < canvas.height) {
    ...
    slice.height = Math.min(pageHInCanvasPx, canvas.height - y);
    ...
    if (!first) pdf.addPage([PX_PER_PAGE.w, PX_PER_PAGE.h]);
    pdf.addImage(img, 'JPEG', 0, 0, pageW, Math.round(slice.height * scale));
    first = false;
    y += slice.height;
  }
  pdf.save(filename);
